...
function renderMarkdown(text: string): string {
  return text
    ...
    .replace(/^[-•]\s+(.+)$/gm, '<li style="margin:0;line-height:1.4">$1</li>')
    .replace(/((?:<li>[\s\S]*?<\/li>\s*)+)/, '<ul style="margin:4px 0;padding-left:1rem;list-style-type:disc">$1</ul>')
    .replace(/\n{2,}/g, '</p><p style="margin:2px 0">')
    .replace(/\n/g, '<br />')
}
